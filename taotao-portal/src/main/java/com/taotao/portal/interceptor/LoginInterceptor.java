...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		//在Handler执行之前处理。返回值决定handler是否执行  true：执行 ，false：不执行
		//1、判断用户是否登陆，从cookie中取token
		...
		if (null == user) {
			...
			//linux中的地址 ：response.sendRedirect(userService.SSO_DOMAIN_BASE_USRL + userService.SSO_PAGE_LOGIN + "?redirect=" + request.getRequestURL());
